...
export async function getDays(
    ...
    const response = await fetch(
        `https://isw.magister.net/api/personen/${userId}/afspraken?status=1&tot=${dateToFormat(
            to
        )}&van=${dateToFormat(from)}`,
        {
            headers: {
    ...
    data.Items.forEach((item: any) => {
        let day = days.find((entry) => {
            if (!entry?.day) return false;
                ...
                return (
                    new Date(entry.day).toDateString() ===
    ...
    days = days.map((day) => {
        ...
        day.subjects = day.subjects?.map((subject: any, i: number) => {
            ...
            if (typeof hour !== "number" || typeof location !== "number")
                return subject;
            ...
            if (hour === 4 && !smallBreak1 && !minirooster) {
                ...
                // Small Break
                smallBreak2 = true;
                if (subjects[i - 1]?.Eind === subject.Start)
                    subjects[i - 1].Einde -= 900000;
                subject.stroom = "2";
                smallBreak();
            }
